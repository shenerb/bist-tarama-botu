    ...
    with st.spinner("Hisseler taranıyor..."):
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                ">
                    <h3 style="margin-bottom:5px;">{hisse}</h3>
                    <small style="color:gray;">Tarih: {row['Tarih']}</small>
                    <p style="margin:8px 0; font-size:18px;">
                        Kapanış: <b>{row['Kapanış']}</b> 
                        <span style='color:{color}; font-weight:bold; margin-left:10px;'>{sign} {abs(row['Değişim'])}%</span>
                    </p>
