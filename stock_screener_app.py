...
# ----- TEMELOZET EXCEL'DEN HALKA AÇIKLIK ORANI OKUMA -----
@st.cache_data
def load_halaciklik_data():
    df_ozet = pd.read_excel("temelozet.xlsx")
    # Kod sütununu uppercase yap ve boşlukları temizle
    df_ozet["Kod"] = df_ozet["Kod"].str.strip().str.upper()
    # Halka Açıklık Oranı sütununu alın, eksik varsa NaN kalır
    ...
    with st.spinner("Hisseler taranıyor..."):
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                # Dolaşımdaki lot bilgisini getiriyoruz:
                lot = dolasim_lot_dict.get(hisse, "N/A")
                if lot != "N/A":
                    lot = f"{int(lot):,}".replace(",", ".")

                # HALKA AÇIKLIK ORANI'nı getiriyoruz:
