...
from tickers import get_all_bist_tickers  # BIST hisse kodlarını sağlayan modül
...
def scan_stocks(tickers, ma_tolerance, volume_threshold, use_ma, use_volume, use_rsi=False, rsi_threshold=30, ceiling_threshold=None, use_buy_filter=False):
    ...
    for ticker in tickers:
        try:
            ...
            # MA filtresi: fiyat, ortalamalara göre tolerans içinde mi?
            ...
            continue
        time.sleep(0.1)  # API limitleri için kısa bekleme
    ...
    with st.spinner("Hisseler taranıyor..."):
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                number_of_analysts_str = str(number_of_analysts) if number_of_analysts else "N/A"
                ...
                <div style="border:1px solid #ccc; border-radius:10px; padding:10px; margin:10px 0;">
                    ...
                    Analist Görüşü: <b>{recommendation_str}</b><br>
                    Analist Sayısı: <b>{number_of_analysts_str}</b>
                </div>
