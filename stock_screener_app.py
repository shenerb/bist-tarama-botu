...
def calculate_trin(data, period=14):
    ...
    adv_volume = df['Volume'].where(df['Advancing'], 0)
    dec_volume = df['Volume'].where(df['Declining'], 0)

    adv_count = df['Advancing'].rolling(window=period).sum()
    dec_count = df['Declining'].rolling(window=period).sum()
    adv_volume_sum = adv_volume.rolling(window=period).sum()
    dec_volume_sum = dec_volume.rolling(window=period).sum()

    epsilon = 1e-10
    adv_count = adv_count.replace(0, epsilon)
    dec_count = dec_count.replace(0, epsilon)
    adv_volume_sum = adv_volume_sum.replace(0, epsilon)
    dec_volume_sum = dec_volume_sum.replace(0, epsilon)

    trin = (adv_count / dec_count) / (adv_volume_sum / dec_volume_sum)
    trin = trin.reindex(df.index)
    return trin
...
def prepare_data_for_plot(ticker):
    ...
    if st.session_state.get("show_trin", False):
        trin_series = calculate_trin(data)
        if trin_series is not None and len(trin_series) == len(data):
            data["TRIN"] = trin_series
        else:
            data["TRIN"] = pd.Series([None]*len(data), index=data.index)
    else:
        data["TRIN"] = pd.Series([None]*len(data), index=data.index)
...
def plot_stock_chart(data, ticker_name):
    if st.session_state.get("show_trin", False) and data["TRIN"].notna().any():
