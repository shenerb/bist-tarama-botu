...
def generate_commentary(rsi, volume_ratio, close, ma20, ma50, ema89, support, resistance):
    commentary = []
    if rsi >= 70:
    ...
    commentary.append(f"EMA89 şu an: {ema89:.2f}")
    commentary.append(f"Basit Destek noktası: {support:.2f}")
    commentary.append(f"Basit Direnç noktası: {resistance:.2f}")
...
def plot_stock_chart(data, ticker_name):
    ...
    ax1.plot(data.index, data["EMA89"], label="EMA89", color="purple", linestyle='--')
...
def scan_stocks(tickers, ma_tolerance, volume_threshold, use_ma, use_volume, use_rsi=False, rsi_threshold=30, ceiling_threshold=None):
    ...
    for ticker in tickers:
        try:
            ...
            ema89_latest = data["EMA89"].iloc[-1]
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                <div style="border:1px solid #ccc; border-radius:10px; padding:10px; margin:10px 0;">
                    ...
                    MA20: {row['MA20']} | MA50: {row['MA50']} | EMA89: {row['EMA89']}<br><br>
                ...
                if data_plot is not None:
                    ...
                    ema89_latest = data_plot["EMA89"].iloc[-1]

                    last_14 = data_plot.tail(14)
                    support = (last_14["Low"].min() + data_plot["Close"].iloc[-1]) / 2
                    resistance = (last_14["High"].max() + data_plot["Close"].iloc[-1]) / 2
