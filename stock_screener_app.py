...
def plot_stock_chart(data, ticker_name):
    # Destek ve dirençleri 20 günlük local min/max ile bul
    support = data['Close'].rolling(window=20, center=True).min().dropna()
    resistance = data['Close'].rolling(window=20, center=True).max().dropna()
    ...
    fig.add_trace(go.Scatter(x=data.index, y=data["MA200"], name='MA200', line=dict(color='red')))

    fig.add_trace(go.Scatter(x=support.index, y=support.values,
                             mode='lines', name='Destek', line=dict(color='green', width=1, dash='dot')))
    fig.add_trace(go.Scatter(x=resistance.index, y=resistance.values,
                             mode='lines', name='Direnç', line=dict(color='red', width=1, dash='dot')))

    fig.update_layout(
        title=f"{ticker_name} - Kapanış, Ortalamalar ve Destek/Direnç",
        xaxis_title="Tarih",
