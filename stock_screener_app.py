...
if st.button("🔍 Taramayı Başlat"):
    with st.spinner("Hisseler taranıyor..."):
        ...
        df, trin = scan_stocks(tickers_to_scan, ma_tolerance, volume_threshold, use_ma, use_volume, use_rsi, rsi_threshold, ceiling_threshold)

        if trin is not None:
            st.markdown(f"### 📊 TRIN Değeri: **{trin:.3f}**")
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                <div style="border:1px solid #ddd; padding:10px; margin-bottom:8px; border-radius:6px">
                    <h3>{hisse} <small style="color:gray; font-weight:normal">({row['Tarih']})</small></h3>
                    <p>Kapanış: {row['Kapanış']} TL &nbsp;&nbsp; <span style="color:{color}; font-weight:bold">{sign} {abs(row['Değişim']):.2f}%</span></p>
                    <p>MA20: {row['MA20']} &nbsp;&nbsp; MA50: {row['MA50']}</p>
                    <p>Hacim Katsayısı: {row['Hacim Katsayısı']} &nbsp;&nbsp; RSI: {row['RSI']}</p>
                    <p>Dolaşımdaki Lot: {lot} &nbsp;&nbsp; Halka Açıklık: {halka_aciklik}</p>
                    <p>Piyasa Değeri (TRY): {market_cap_try if market_cap_try else 'N/A'} &nbsp;&nbsp; Piyasa Değeri (USD): {market_cap_usd_str}</p>
                </div>
