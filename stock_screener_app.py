...
def generate_commentary(rsi, volume_ratio, close, ma20, ma50, macd, macd_signal, price, upper_bb, lower_bb):
    commentary = []
    ...
    # MACD Yorumu
    if macd > macd_signal:
        commentary.append("MACD sinyal hattının üzerinde, pozitif momentum mevcut.")
    else:
        commentary.append("MACD sinyal hattının altında, momentum zayıf olabilir.")

    # Bollinger Bandı Yorumu
    if price > upper_bb:
        commentary.append("Fiyat üst Bollinger bandını aştı, aşırı alım sinyali olabilir.")
    elif price < lower_bb:
        commentary.append("Fiyat alt Bollinger bandının altında, aşırı satım sinyali olabilir.")
    else:
        commentary.append("Fiyat Bollinger bandı içinde, normal dalgalanma.")
...
def plot_stock_chart(data, ticker_name):
    ...
    # Bollinger Bandları
    plt.plot(data.index, data["UpperBB"], label="Upper BB", color="gray", linestyle="--")
    plt.plot(data.index, data["LowerBB"], label="Lower BB", color="gray", linestyle="--")

    plt.title(f"{ticker_name} - Son 1 Yıl Kapanış, MA ve Bollinger Bandları")
    plt.legend()
...
def prepare_data_for_plot(ticker):
    ...
    ma, upper_bb, lower_bb = calculate_bollinger_bands(data["Close"])
    data["MA20_BB"] = ma  # Opsiyonel
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                if data_plot is not None:
                    ...
                    macd_latest = data_plot["MACD"].iloc[-1]
                    macd_signal_latest = data_plot["MACD_Signal"].iloc[-1]
                    close_latest = data_plot["Close"].iloc[-1]
                    upper_bb = data_plot["UpperBB"].iloc[-1]
                    lower_bb = data_plot["LowerBB"].iloc[-1]
                    ma20_latest = data_plot["MA20"].iloc[-1]
                    ma50_latest = data_plot["MA50"].iloc[-1]
                    volume_ratio = row['Hacim Katsayısı']
                    ...
                    st.markdown(f"<b>Teknik Yorum:</b> {commentary} <br><i>Yatırım tavsiyesi değildir.</i>", unsafe_allow_html=True)
