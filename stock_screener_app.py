...
if st.button("🔍 Taramayı Başlat"):
    with st.spinner("Tüm BIST hisseleri taranıyor..."):
        ...
        if df.empty:
            ...
            # ✅ Autocomplete hisse arama
            search_input = st.text_input("🔎 Hisse ara (yazmaya başlayın):").upper()
            filtered_df = df[df["Hisse"].str.contains(search_input, na=False)] if search_input else df
