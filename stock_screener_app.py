...
def plot_stock_chart(data, ticker_name):
    ...
    # BAY-P yazısı grafiğin üst sağ köşesine dikkat çekici şekilde
    ax1.text(
        0.95, 0.95, "Bay-P",
...
def scan_stocks(tickers, ma_tolerance, volume_threshold, use_ma, use_volume_filter, use_rsi=False, rsi_threshold=30, ceiling_threshold=None):
    results = []
    for ticker in tickers:
        try:
            ...
            passes_volume = (volume_ratio >= volume_threshold) if use_volume_filter else True
...
use_volume_filter = st.sidebar.checkbox("Hacim Filtresi Kullan", value=True)
...
if st.button("🔍 Taramayı Başlat"):
    with st.spinner("Hisseler taranıyor..."):
        ...
        df = scan_stocks(tickers_to_scan, ma_tolerance, volume_threshold, use_ma, use_volume_filter, use_rsi, rsi_threshold, ceiling_threshold)
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                # Değişim yüzdesi renk ayarı
                change_color = "green" if row['Değişim'] >= 0 else "red"
                arrow = "▲" if row['Değişim'] >= 0 else "▼"

                st.markdown(f"""
                <div style="border:1px solid #ccc; border-radius:10px; padding:10px; margin:10px 0;">
                    <strong>{hisse}</strong><br>
                    <i>Tarih: {row['Tarih']}</i><br>
                    Kapanış: <b>{row['Kapanış']}</b> 
                    (<span style="color:{change_color}; font-weight:bold;">{arrow} {abs(row['Değişim'])}%</span>)<br>
            ...
            st.markdown("""
            <hr>
            <p style="font-size:12px; color:gray;"><em>Yatırım tavsiyesi değildir.</em></p>
            """, unsafe_allow_html=True)
