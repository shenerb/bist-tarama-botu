...
5. Piyasa geneli TRIN analizi için **📊 Piyasa TRIN Analizi** butonuna tıklayın.
""")
...
def calculate_trin_for_market(tickers, start="2024-01-01", end=None):
    ...
    for date in all_dates:
        adv_issues = dec_issues = adv_vol = dec_vol = 0
        for df in all_data.values():
            ...
            if isinstance(row, pd.DataFrame):
                row = row.iloc[0]
            prev_close = row.at['Prev_Close']
            if pd.isna(prev_close):
                continue
            if row.at['Close'] > prev_close:
                adv_issues += 1
                adv_vol += row.at['Volume']
            elif row.at['Close'] < prev_close:
                dec_issues += 1
                dec_vol += row.at['Volume']

        if adv_issues > 0 and dec_issues > 0 and adv_vol > 0 and dec_vol > 0:
            trin = (adv_issues / dec_issues) / (adv_vol / dec_vol)
            trin_series.append({"Date": date, "TRIN": trin})

    return pd.DataFrame(trin_series).set_index("Date")

def calculate_trin_rsi(trin_df, period=14):
    trin_df["TRIN_RSI"] = calculate_rsi(trin_df["TRIN"], period=period)
    return trin_df
...
if st.sidebar.button("📊 Piyasa TRIN Analizi"):
    with st.spinner("Piyasa TRIN hesaplanıyor..."):
        ...
        trin_df = calculate_trin_rsi(trin_df, period=14)
