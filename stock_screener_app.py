...
# BIST hisse senedi listesini getiren fonksiyon (örnek)
def get_all_bist_tickers():
    # Gerçek ortamda buraya BIST hisselerini çekme kodu veya sabit liste koyabilirsin.
    # Örnek olarak birkaç BIST hissesi:
    return ["AKBNK.IS", "ASELS.IS", "BIMAS.IS", "FROTO.IS", "GARAN.IS", "KOZAL.IS"]
...
# MACD yorumlama - son 'lookback' bar için sinyal değişimi kontrolü
def generate_macd_commentary(macd_line, signal_line, lookback=2):
    if len(macd_line) < lookback + 1:
        return "MACD veri yetersiz."
    if macd_line.iloc[-1] > signal_line.iloc[-1] and macd_line.iloc[-lookback-1] <= signal_line.iloc[-lookback-1]:
        return "MACD Al sinyali verdi."
    elif macd_line.iloc[-1] < signal_line.iloc[-1] and macd_line.iloc[-lookback-1] >= signal_line.iloc[-lookback-1]:
...
# Grafik için veri hazırlama (1 yıl, günlük)
...
macd_lookback = 2  # MACD sinyal değişimini kontrol ederken kaç bar geriye bakılacak
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                if data_plot is not None:
                    ...
                    macd_comment = generate_macd_commentary(data_plot["MACD_Line"], data_plot["MACD_Signal"], lookback=macd_lookback)

                    commentary = generate_commentary(
                        rsi_latest,
                        data_plot["Volume"].iloc[-1] / data_plot["Volume"].rolling(20).mean().iloc[-1],
                        data_plot["Close"].iloc[-1],
                        data_plot["MA20"].iloc[-1],
                        data_plot["MA50"].iloc[-1]
                    )
