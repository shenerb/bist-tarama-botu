...
# AL/SAT sinyali üretimi
def generate_trade_signal(rsi, macd_line, signal_line, close, ema):
    if rsi < 30 and macd_line > signal_line and close > ema:
        return "AL"
    elif rsi > 70 and macd_line < signal_line and close < ema:
        return "SAT"
    else:
        return "NÖTR"
...
def scan_stocks(tickers, ma_tolerance, volume_threshold, use_ma, use_rsi=False, rsi_threshold=30):
    ...
    for ticker in tickers:
        try:
            ...
            trade_signal = generate_trade_signal(rsi_latest, macd_latest, signal_latest, close, ema_latest)
