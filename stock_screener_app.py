...
# --- KAP Son Bildirimini Çekme Fonksiyonu ---
def fetch_latest_kap_news(ticker):
    base_url = "https://www.kap.org.tr/tr/BildirimSorgulari/SirketBildirimleri"
    params = {
        "sirketKodu": ticker.replace(".IS", ""),
        "submit": "Arama"
    }
    ...
    link = "https://www.kap.org.tr" + cols[1].find("a")["href"]
    ...
    with st.spinner("Tüm BIST hisseleri taranıyor..."):
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                # KAP'dan son bildirimi çek ve göster
                kap_news = fetch_latest_kap_news(ticker_full)
                if kap_news:
                    st.markdown(f"""
                        <b>KAP Son Bildirim ({kap_news['date']}):</b> <a href="{kap_news['link']}" target="_blank">{kap_news['title']}</a>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown("<i>KAP bildirimi bulunamadı.</i>", unsafe_allow_html=True)
