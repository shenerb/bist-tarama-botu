...
# 🌐 Finnhub API fonksiyonu (geliştirilmiş, hata kontrolü ve debug ile)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "metric" in data:
                ...
                st.error(f"Finnhub API'den metric verisi alınamadı: {data}")
                return None
        else:
            st.error(f"Finnhub API hatası: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        st.error(f"Finnhub API isteğinde hata oluştu: {e}")
        return None
...
api_key = st.sidebar.text_input("🔑 Finnhub API Anahtarınız", type="password")  # <-- Finnhub API input
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                if data_plot is not None:
                    ...
                    # 📊 Finnhub Finansal Veriler
                    if api_key:
                        fin_data = get_finnhub_fundamentals(ticker_full, api_key)
                        if fin_data:
                            st.markdown(f"""
                                <div style="border:1px dashed #ccc; padding:8px; margin-top:10px;">
                                <b>📊 Finansal Veriler</b><br>
                                F/K (TTM): <b>{fin_data.get('peNormalizedAnnual', 'N/A')}</b><br>
                                PD/DD: <b>{fin_data.get('pbAnnual', 'N/A')}</b><br>
                                Borç/Özsermaye: <b>{fin_data.get('debtEquityAnnual', 'N/A')}</b><br>
                                Net Kâr Marjı: <b>{fin_data.get('netProfitMarginAnnual', 'N/A')}%</b><br>
                                Temettü Verimi: <b>{fin_data.get('dividendYieldIndicatedAnnual', 'N/A')}%</b>
                                </div>
                            """, unsafe_allow_html=True)
                        else:
                            st.info(f"{row['Hisse']} için finansal veri bulunamadı veya API limiti aşılmış olabilir.")

                st.markdown("</div>", unsafe_allow_html=True)

        if not api_key:
            st.info("💡 Finansal oranların görünmesi için sağdaki alana Finnhub API anahtarınızı girin.")
