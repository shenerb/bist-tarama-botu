...
st.set_page_config(page_title="BIST Dip & Hacim Tarayıcı", layout="wide")
...
def plot_stock_chart(data, ticker_name):
    ...
    for date, level in support:
        plt.axhline(level, linestyle="--", color="green", alpha=0.3)
    for date, level in resistance:
        plt.axhline(level, linestyle="--", color="red", alpha=0.3)

    plt.title(f"{ticker_name} - Teknik Grafik", fontsize=14)
    plt.legend()
    plt.tight_layout()
    st.pyplot(plt)
    plt.clf()
    ...
    ax_macd.plot(data.index, macd, label="MACD", color="purple")
    ax_macd.plot(data.index, signal, label="Signal", color="magenta")
