...
from tickers import get_all_bist_tickers  # Kendi modülün, doğru olduğundan emin ol
...
def get_yfinance_fundamentals(ticker):
    try:
        ...
        return {"pe_ratio": "N/A", "pb_ratio": "N/A"}
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                if data_plot is not None:
                    ...
                    commentary = generate_commentary(rsi_latest, row['Hacim Katsayısı'], row['Kapanış'], ma20_latest, ma50_latest)
                    st.markdown(f"<i>{commentary}</i>", unsafe_allow_html=True)
                    plot_stock_chart(data_plot, ticker_full)
                else:
                    st.info("Grafik için yeterli veri yok.")

                # Finansal oranları getir
                fundamentals = get_yfinance_fundamentals(ticker_full)
                pe = fundamentals['pe_ratio']
                pb = fundamentals['pb_ratio']

                st.markdown(f"""
                    <b>F/K (PE) Oranı:</b> {pe}<br>
                    <b>PD/DD (PB) Oranı:</b> {pb}
                    </div>
                """, unsafe_allow_html=True)
