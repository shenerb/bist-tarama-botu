...
def plot_stock_chart(data, ticker_name):
    ...
    plt.plot(data.index, data["Close"], label="Kapanış", color="blue", linewidth=1.5)
    ...
    plt.plot(data.index, data["MA200"], label="MA200", color="red", linestyle="--")

    # Destek ve direnç
    support = data["Close"].min()
    resistance = data["Close"].max()
    plt.axhline(support, color="purple", linestyle=":", linewidth=1.2, label=f"Destek: {support:.2f}")
    plt.axhline(resistance, color="brown", linestyle=":", linewidth=1.2, label=f"Direnç: {resistance:.2f}")

    plt.title(f"{ticker_name} - 1 Yıllık Grafik (Kapanış & MA & Destek/Direnç)")
