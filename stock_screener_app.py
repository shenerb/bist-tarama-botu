...
# MACD yorumlama - lookback ile son x gün kontrolü
def generate_macd_commentary(macd_line, signal_line, lookback=5):
    recent_macd = macd_line[-lookback:]
    recent_signal = signal_line[-lookback:]

    for i in range(1, lookback):
        if recent_macd.iloc[i] > recent_signal.iloc[i] and recent_macd.iloc[i-1] <= recent_signal.iloc[i-1]:
            return f"MACD Al sinyali verdi ({lookback} gün içinde)."
        elif recent_macd.iloc[i] < recent_signal.iloc[i] and recent_macd.iloc[i-1] >= recent_signal.iloc[i-1]:
            return f"MACD Sat sinyali verdi ({lookback} gün içinde)."
...
macd_lookback = st.sidebar.slider("MACD Sinyal Gün Aralığı", 1, 10, 5)
