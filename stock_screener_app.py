...
def scan_stocks(tickers, ma_tolerance, volume_threshold, use_ma, use_volume, use_rsi=False, rsi_threshold=30, ceiling_threshold=None,
                min_pddd=0.0, max_pddd=10.0, min_marketcap=0.0, max_marketcap=1e9):
    results = []
    for ticker in tickers:
        try:
            ...
            if pddd is None or market_cap_try is None or usdtry is None:
                continue

            market_cap_usd = market_cap_try / usdtry / 1e6  # Milyon $

            if not (min_pddd <= pddd <= max_pddd and min_marketcap <= market_cap_usd <= max_marketcap):
                continue
...
# Yeni filtreler: PD/DD ve Piyasa Değeri
min_pddd = st.sidebar.number_input("Min PD/DD", value=0.0)
max_pddd = st.sidebar.number_input("Max PD/DD", value=10.0)
min_marketcap = st.sidebar.number_input("Min Piyasa Değeri (Milyon $)", value=0.0)
max_marketcap = st.sidebar.number_input("Max Piyasa Değeri (Milyon $)", value=100000.0)
...
if st.button("🔍 Taramayı Başlat"):
    with st.spinner("Hisseler taranıyor..."):
        ...
        df = scan_stocks(
            tickers_to_scan, ma_tolerance, volume_threshold, use_ma, use_volume,
            use_rsi, rsi_threshold, ceiling_threshold,
            min_pddd, max_pddd, min_marketcap, max_marketcap
        )
