...
# ---------------------- TRIN Hesaplama Fonksiyonu ----------------------

def calculate_trin(tickers):
    """
    TRIN = (Yükselen Hisse Sayısı / Düşen Hisse Sayısı) /
           (Yükselen Hacim / Düşen Hacim)
    """

    try:
        data = yf.download(tickers, period="1d", interval="1d", progress=False)
    except Exception as e:
        return None, f"Veri indirirken hata: {str(e)}"

    if data.empty:
        return None, "Veri yok"

    # Veri yapısını düzelt (tek veya çoklu ticker farkı)
    if isinstance(data.columns, pd.MultiIndex):
        close = data['Close'].iloc[-1]
        volume = data['Volume'].iloc[-1]
    else:
        # Tek ticker için uyarlama
        close = pd.Series([data['Close'][-1]], index=[tickers[0]])
        volume = pd.Series([data['Volume'][-1]], index=[tickers[0]])

    # Hisselerin önceki kapanışı yoksa sıfır kabul
    prev_day_data = yf.download(tickers, period="2d", interval="1d", progress=False)
    if prev_day_data.empty:
        return None, "Önceki gün verisi yok"

    if isinstance(prev_day_data.columns, pd.MultiIndex):
        prev_close = prev_day_data['Close'].iloc[-2]
    else:
        prev_close = pd.Series([prev_day_data['Close'][-2]], index=[tickers[0]])

    # Hisselerin günlük değişimine göre yükselen ve düşenleri bul
    advancing_stocks = []
    declining_stocks = []
    for ticker in tickers:
        try:
            c = close.get(ticker, None)
            pc = prev_close.get(ticker, None)
            if c is None or pc is None:
                continue
            if c > pc:
                advancing_stocks.append(ticker)
            elif c < pc:
                declining_stocks.append(ticker)
        except:
            continue

    adv_count = len(advancing_stocks)
    dec_count = len(declining_stocks)

    # Hacimleri topla
    adv_volume = volume[advancing_stocks].sum() if adv_count > 0 else 0
    dec_volume = volume[declining_stocks].sum() if dec_count > 0 else 0

    if dec_count == 0 or dec_volume == 0:
        return None, "Yükselen veya düşen hisse sayısı/hacmi sıfır"
...
# Yeni: TRIN tarama modu checkbox
use_trin_scan = st.sidebar.checkbox("TRIN Tarama Modu", value=False)
...
if st.button("🔍 Taramayı Başlat"):
    if use_trin_scan:
        st.info("TRIN hesaplanıyor...")
        tickers_to_scan = selected_tickers if selected_tickers else all_tickers
        trin_value, error = calculate_trin(tickers_to_scan)
        if error:
            st.error(f"TRIN hesaplanamadı: {error}")
        else:
            color = "green" if trin_value < 1 else "red"
            yorum = "Olumlu: Alım baskısı hakim" if trin_value < 1 else "Olumsuz: Satış baskısı hakim"
            st.markdown(f"""
                <h3>TRIN Değeri: <span style='color:{color}'>{trin_value:.3f}</span></h3>
                <p style='color:{color}; font-weight:bold;'>{yorum}</p>
            """, unsafe_allow_html=True)
    else:
        # Mevcut tarama fonksiyonun aynen çalışacak
        with st.spinner("Hisseler taranıyor..."):
            ...
            if df.empty:
                ...
                # Sonuç ekranı hiç değiştirilmedi
                for _, row in df.iterrows():
                    st.write(f"{row['Hisse']} - Kapanış: {row['Kapanış']} - Değişim: %{row['Değişim']} - RSI: {row['RSI']}")
                    # İstersen buraya grafik ve detay eklemeye devam edebilirsin
