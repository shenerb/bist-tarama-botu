...
def plot_stock_chart(data, ticker_name):
    ...
    fig.add_trace(go.Scatter(x=data.index, y=data['Close'], mode='lines', name='Kapanış', line=dict(color='blue')))
    ...
    for date, level in supports[-3:]:
        fig.add_hline(y=level, line=dict(color='gray', dash='dot'), annotation_text='Destek', annotation_position="bottom left")

    for date, level in resistances[-3:]:
        fig.add_hline(y=level, line=dict(color='black', dash='dot'), annotation_text='Direnç', annotation_position="top left")

    fig.update_layout(title=f"{ticker_name} - Kapanış ve Ortalama (Destek/Direnç)",
