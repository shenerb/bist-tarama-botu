...
def plot_stock_chart(data, ticker_name):
    ...
    ax1.plot(data.index, data["MA200"], label="MA200", color="red", linestyle="--")
    ax1.set_title(f"{ticker_name} - Son 1 Yıl Fiyat ve Ortalamalar")
    ax1.legend()
    ax1.grid(True)

    data["AvgVolume20"] = data["Volume"].rolling(20).mean()
    colors = ["green" if v > 1.5 * avg else "gray" for v, avg in zip(data["Volume"], data["AvgVolume20"])]
    ax2.bar(data.index, data["Volume"], color=colors, label="Hacim")
    ax2.set_ylabel("Hacim")
    ax2.legend()
    ax2.grid(True)

    fig.text(0.5, 0.5, "Bay-P", fontsize=40, color="gray", alpha=0.12,
