...
def macd_commentary(macd_line, signal_line):
    if macd_line.iloc[-1] > signal_line.iloc[-1]:
        return "MACD Al sinyali veriyor."
    elif macd_line.iloc[-1] < signal_line.iloc[-1]:
        return "MACD Sat sinyali veriyor."
    else:
        return "MACD belirsiz durumda."
...
def prepare_data_for_plot(ticker):
    ...
    # Supertrend yönü (True=Al, False=Sat)
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                if data_plot is not None:
                    ...
                    # MACD yorum
                    macd_text = macd_commentary(data_plot["MACD_Line"], data_plot["Signal_Line"])
                    st.markdown(f"<b>MACD:</b> {macd_text}", unsafe_allow_html=True)

                    # Supertrend yorum (son veri için)
                    supertrend_dir = data_plot["Supertrend_Direction"].iloc[-1]
                    supertrend_text = "Supertrend: AL sinyali." if supertrend_dir else "Supertrend: SAT sinyali."
                    st.markdown(f"<b>{supertrend_text}</b>", unsafe_allow_html=True)
