...
# --- Hisse Adı ile Arama Özelliği ---

st.markdown("---")
st.subheader("🔎 Hisse Bazlı Arama")

user_input = st.text_input("İncelemek istediğiniz hisse kodunu girin (örn: ASELS):")

if user_input:
    ticker_symbol = user_input.upper().strip() + ".IS"
    st.info(f"{user_input.upper()} verileri alınıyor...")

    data_plot = prepare_data_for_plot(ticker_symbol)

    if data_plot is not None:
        rsi_latest = data_plot["RSI"].iloc[-1]
        macd_comment = generate_macd_commentary(data_plot["MACD_Line"], data_plot["MACD_Signal"])
        close_price = data_plot["Close"].iloc[-1]
        ...
        st.markdown(f"""
            **Kapanış:** {close_price:.2f}  
            **RSI:** {rsi_latest:.2f}  
            **MACD:** {macd_comment}  
            **Hacim / Ortalama:** {volume_ratio:.2f}  
            **MA20:** {ma20:.2f}  
            **MA50:** {ma50:.2f}  
            ---
            *{commentary}*
        ...
        plot_stock_chart(data_plot, user_input.upper())

    else:
        st.error("Veri alınamadı veya hisse kodu hatalı olabilir.")
