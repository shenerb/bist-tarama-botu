...
# BIST 100 listesini çeker
@st.cache_data(ttl=86400)
def get_bist100_tickers():
    url = "https://www.isyatirim.com.tr/_layouts/15/ISYatirim.Website/Common/Data.aspx/GetBIST100"
    r = requests.get(url)
    df = pd.DataFrame(r.json()["value"])
    return [symbol + ".IS" for symbol in df["Code"].tolist()]
...
if st.button("🔍 Taramayı Başlat"):
    with st.spinner("Tüm BIST 100 hisseleri taranıyor..."):
